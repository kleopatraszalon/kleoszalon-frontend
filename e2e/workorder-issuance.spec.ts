...
test('recepciós munkalap kiadás: időpont → fizetés → készlet → archiválás → PDF', async ({ page, context, request }) => {
  ...
  await expect(page.getByText('E2E Vendég')).toBeVisible();
  ...
  // A PDF-gomb idempotensen újrahívja a finalize végpontot. Ennek nem szabad
  // még egyszer készletet fogyasztania vagy új munkalapot létrehoznia.
