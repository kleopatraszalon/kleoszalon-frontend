...
test("VIR Intelligence is management protected and linked from Manager Cockpit",()=>{const r=read("src/routing/adminRoutes.tsx"),c=read("src/pages/VirManagerCockpitPage.tsx");expect(r).toContain('{ path: "/admin/vir/intelligence", element: R(MANAGEMENT, <VirIntelligencePage />) }');expect(c).toContain('/admin/vir/intelligence');expect(c).toContain('Alap üzleti intelligencia');expect(c).toContain('Core Business Intelligence')});

test("profitability UI explains direct-cost scope and canonical engine without overstating net profit",()=>{const p=read("src/pages/VirIntelligencePage.tsx");for(const marker of ["Közvetlen fedezet","receptúra szerinti anyag − időarányos munkabér − jutalék","Kanonikus számítás:","Munkáltatói közterhek és általános rezsi még nem részei"])expect(p).toContain(marker)});
