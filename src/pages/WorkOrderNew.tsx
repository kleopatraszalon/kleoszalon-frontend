...
type Employee = {
  id: string;
  name: string; // full_name backendről átnevezve
  role: string;
};
...
type ServiceCategory = {
  category: string; // pl. "Gél lakk", "Hajvágás", stb.
...
type WorkOrderDraft = {
  visit_status: "várakozik" | "megérkezett" | "nem_jött_el" | "megerősítve";
  record_note: string;
  employee_id: string;
  client: ClientInfo;
  selectedServices: {
...
function WorkOrderNew() {
  ...
  // kik hozhatnak létre munkalapot
  ...
  // ─────────── Dolgozók betöltése az employees táblából ───────────
  // Backend oldalról majd azt várjuk, hogy visszaadja:
  // [{ id, name, role }]
  // ahol name = employees.full_name
  useEffect(() => {
    fetch("http://localhost:5000/api/employees", {
      ...
      .catch((err) => console.error("Dolgozók betöltése hiba:", err));
  }, [token]);

  // ─────────── Szolgáltatások betöltése a services táblából ───────────
  // Backend itt kategóriákra bontva ad vissza:
  // [
  //   {
  //     category: "Gél lakk",
  //     services: [
  ...
  // ─────────── Szolgáltatás hozzáadása a munkalaphoz ───────────
  ...
  const removeServiceFromDraft = (service_id: string) => {
    setDraft((prev) => ({
      ...
      selectedServices: prev.selectedServices.filter(
        (svc) => svc.service_id !== service_id
      ),
    }));
  };

  // ─────────── Státusz állítása ───────────
  const setVisitStatus = (
    status: "várakozik" | "megérkezett" | "nem_jött_el" | "megerősítve"
  ) => {
  ...
  // ─────────── Mentés backendre ───────────
  // POST /api/workorders
  // payload kompatibilis a backenddel
  ...
  // ─────────── Státusz gomb design ───────────
  const statusBtnClass = (value: string) =>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <section className="bg-white dark:bg-neutral-800 rounded-lg shadow p-4 w-full md:w-1/4 flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  className={statusBtnClass("várakozik")}
                  onClick={() => setVisitStatus("várakozik")}
                >
                  Várakozás az ügyfélre
                </button>
                <button
                  className={statusBtnClass("megérkezett")}
                  onClick={() => setVisitStatus("megérkezett")}
                >
                  Ügyfél megérkezett
                </button>
                <button
                  className={statusBtnClass("nem_jött_el")}
                  onClick={() => setVisitStatus("nem_jött_el")}
                >
                  Nem jött el
                </button>
                <button
                  className={statusBtnClass("megerősítve")}
                  onClick={() => setVisitStatus("megerősítve")}
                >
                  Megerősítve
                </button>
              </div>
            ...
            <section className="bg-white dark:bg-neutral-800 rounded-lg shadow p-4 w-full md:w-2/4 flex flex-col gap-4">
              ...
              <div className="flex border rounded-lg overflow-hidden text-sm font-medium text-gray-700 dark:text-gray-200 dark:border-neutral-700">
                ...
                <button className="flex-1 py-2 text-gray-400 cursor-not-allowed">
                  Termékek
                </button>
              </div>
              ...
              {/* Gyors ajánlott szolgáltatások (az első kategória első 3 tétele) */}
              <div className="flex flex-wrap gap-2">
              ...
              <div className="border rounded-lg bg-gray-50 dark:bg-neutral-900 dark:border-neutral-700 max-h-[300px] overflow-auto text-sm">
                {filteredCategories.map((cat) => (
                  <details
                    key={cat.category}
                    className="border-b last:border-b-0 dark:border-neutral-700"
                  >
              ...
              <div>
                ...
                {draft.selectedServices.length === 0 && (
                  <div className="text-xs text-gray-400 dark:text-gray-500">
                    Még nincs kiválasztva szolgáltatás.
                  </div>
                )}
                {draft.selectedServices.length > 0 && (
                    ...
                    {draft.selectedServices.map((svc) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            removeServiceFromDraft(svc.service_id)
                          }
                        >
                          eltávolít
                        </button>
