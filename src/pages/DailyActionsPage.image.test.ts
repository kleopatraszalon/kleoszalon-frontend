...
test("publish persists a new campaign before sending it",()=>{expect(source).toContain("async function persist");expect(source).toContain("const campaign=await persist()");expect(source).toContain("campaign.id}/publish")});
