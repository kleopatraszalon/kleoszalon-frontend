...
export default function VirManagerCockpitPage(){
  ...
  const load=useCallback(async()=>{
    ...
    try{setData(await getVirManagerCockpit({date,locationId:locationId||undefined}));}
    catch(err:any){setError(err?.response?.data?.error||err?.message||"A Manager Cockpit nem tölthető be.");}
  ...
  const openActions=Number(data?.actions.open_count||0);
  const critical=Number(data?.actions.critical_count||0);

  return <div className="vir-management-page">
    <div className="vir-management-header">
      <div><h1 className="vir-management-title">VIR Manager Cockpit</h1><div className="vir-management-subtitle">Mai üzleti állapot, kockázatok és vezetői teendők egy képernyőn.</div></div>
    ...
    {loading&&!data?<div className="vir-spinner">Vezetői adatok betöltése…</div>:null}
    {data&&<>
      ...
      <div className="vir-management-grid">
        <div>
          <section className="vir-panel"><div className="vir-panel-header"><div><div className="vir-panel-title">Ami ma vezetői figyelmet kér</div><div className="vir-panel-muted">Prioritás, SLA és forrás alapján rendezve.</div></div><button className="vir-link-button" onClick={()=>navigate("/admin/vir/actions")}>Összes teendő →</button></div>
            <div className="vir-action-list">{data.top_actions.length===0?<div className="vir-empty">Nincs nyitott vezetői akció.</div>:data.top_actions.map(item=><article key={item.id} className={`vir-action-card is-${item.priority.toLowerCase()}`}><div className="vir-action-card-head"><div><div className="vir-action-title">{item.title}</div><div className="vir-action-description">{item.description}</div></div>{item.source_route&&<button className="vir-link-button" onClick={()=>navigate(item.source_route!)}>Forrás →</button>}</div><div className="vir-badge-row"><span className={`vir-badge ${item.priority.toLowerCase()}`}>{priorityLabel[item.priority]}</span><span className={`vir-badge ${statusClass[item.status]}`}>{statusLabel[item.status]}</span><span className="vir-badge">{item.source}</span>{item.assignee_name&&<span className="vir-badge">Felelős: {item.assignee_name}</span>}{item.due_at&&<span className="vir-badge">Határidő: {new Date(item.due_at).toLocaleString("hu-HU")}</span>}</div></article>)}</div>
          </section>
        </div>
        <div>
          <section className="vir-panel"><div className="vir-panel-title">Akcióállapot</div><div className="vir-stat-line"><span>Nyitott</span><strong>{data.actions.open_count}</strong></div><div className="vir-stat-line"><span>Kritikus</span><strong>{data.actions.critical_count}</strong></div><div className="vir-stat-line"><span>Magas prioritás</span><strong>{data.actions.high_count}</strong></div><div className="vir-stat-line"><span>Lejárt</span><strong>{data.actions.overdue_count}</strong></div><div className="vir-stat-line"><span>Jóváhagyásra vár</span><strong>{data.actions.approval_count}</strong></div></section>
          <section className="vir-panel"><div className="vir-panel-header"><div className="vir-panel-title">Források terheltsége</div></div><table className="vir-source-table"><thead><tr><th>Forrás</th><th>Nyitott</th><th>Sürgős</th></tr></thead><tbody>{data.source_health.length===0?<tr><td colSpan={3} className="vir-empty">Nincs aktív forrás.</td></tr>:data.source_health.map(row=><tr key={row.source}><td>{row.source}</td><td>{row.open_count}</td><td><strong>{row.urgent_count}</strong></td></tr>)}</tbody></table></section>
        </div>
      </div>
    </>}
  </div>;
