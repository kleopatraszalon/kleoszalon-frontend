...
export default function TenantSettingsPage(){
 ...
 const features=useMemo(()=>{
  const plan={...(tenant?.plan_features||subscription?.features||{})};
 ...
 return <main className="tenantset-page">
  ...
  {tab==="users"&&<section className="tenantset-panel"><h2>Felhasználók</h2><p className="lead">A tenant-tagság határozza meg, hogy egy felhasználó melyik szervezet adataihoz férhet hozzá. A munkatársi funkciójogokat a VIR hozzáférés-kezelője kezeli.</p><div className="tenantset-callout"><UsersRound/><div><b>Tenant-hozzáférés és munkatársi jogosultságok</b><p>Az aktuális felhasználó tenant szerepköre: <strong>{role}</strong>. Munkatársankénti menü-, főoldal- és funkciójogosultságok az Access Control felületen kezelhetők.</p><a href="/admin/access-control">Hozzáférések kezelése →</a></div></div></section>}

  {tab==="roles"&&<section className="tenantset-panel"><h2>Szerepkörök</h2><p className="lead">A VIR két külön védelmi réteget használ: tenant-szerepkör a szervezeti határhoz, üzleti szerepkör a napi funkciókhoz.</p><div className="tenantset-rolegrid"><Role title="owner / admin" text="Tenant szintű adminisztráció, előfizetés és központi beállítások."/><Role title="manager" text="Vezetői operáció és riportok a saját jogosultsági körben."/><Role title="member" text="Normál tenant-tagság; a konkrét funkciókat a munkatársi RBAC adja."/></div><a className="tenantset-link" href="/admin/access-control">Munkatársi szerepkörök és funkciók →</a></section>}
