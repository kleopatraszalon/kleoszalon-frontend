...
export default function VirP4Page(){
 ...
 return <div className="vir-management-page p4-page">
  <section className="p4-hero">
   <div className="p4-hero-copy"><div className="p4-eyebrow">{t('OPERÁCIÓS DÖNTÉSTÁMOGATÁS','OPERATIONS DECISION SUPPORT')}</div><h1 className="vir-management-title">{t('Működési intelligencia','Operations Intelligence')}</h1><p>{t('A munkaerőt, műszakokat, munkatársi bevételt, szolgáltatásportfóliót és a szalonok közötti átfedést egyetlen vezetői döntési felületre rendezi.','Brings workforce, shifts, staff revenue, service portfolio and cross-salon overlap into one management decision surface.')}</p></div>
  ...
  <section className="p4-control-card">
   <div className="p4-control-copy"><small>{active.description}</small></div>
  ...
  {loading?<div className="p4-loading"><div className="p4-skeleton-grid">{[1,2,3,4].map(x=><span key={x}/>)}</div><div className="p4-skeleton-panel"/></div>:<>
   ...
   {!error&&<section className="vir-panel p4-module-panel"><div className="p4-advisory-badge">{t('Javaslat · emberi döntés','Advisory · human decision')}</div>
