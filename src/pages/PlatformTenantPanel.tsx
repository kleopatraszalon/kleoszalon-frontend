...
export default function PlatformTenantPanel(){
 ...
 const createTenant=async(e:React.FormEvent)=>{e.preventDefault();setSaving(true);setError('');setMessage('');try{const r=await axios.post(`${API_BASE}/saas/platform/tenants`,form,authConfig());setForm({slug:'',name:'',legal_name:'',billing_email:'',plan_code:'start',status:'trial'});setMessage(`${r.data?.tenant?.name||'Az új tenant'} létrejött, az onboarding automatikusan elindult.`);await load()}catch(e:any){setError(e?.response?.data?.error||'Az új tenant nem hozható létre.')}finally{setSaving(false)}};
