 ...
 return <div className="saas-admin-page" style={{paddingBottom:0}}>
  <section className="saas-panel">
   ...
   <form onSubmit={createTenant} className="saas-form-grid" style={{marginTop:18}}>
    <div style={{gridColumn:'1/-1',padding:12,border:'1px solid #e5e7eb',borderRadius:12}}><label>Provisioning profil<select value={profileCode} onChange={e=>applyProfile(e.target.value)}>{PROVISIONING_PROFILES.map(profile=><option key={profile.code} value={profile.code}>{profile.label}</option>)}</select></label><small style={{display:'block',marginTop:6}}>{selectedProfile.description}</small></div>
   ...
   <div className="saas-table-wrap" style={{marginTop:18}}><table><thead><tr><th>Tenant</th><th>Csomag</th><th>Státusz</th><th>Provisioning</th><th>Következő lépés</th><th>Első admin</th><th>Telephely</th><th>Felhasználó</th><th>Művelet</th></tr></thead><tbody>{rows.length?rows.map(r=>{const progress=Math.max(0,Math.min(100,Number(r.onboarding_progress||0)));const next=STEP_LABELS[String(r.onboarding_next_step||'')]||String(r.onboarding_next_step||'—');const invite=invitationLabel(r);return <tr key={r.id}><td><b>{r.name}</b><br/><small>{r.slug}</small></td><td>{r.plan_name||r.plan_code||'—'}</td><td><span className={`saas-status-pill status-${r.status}`}>{r.status}</span></td><td>{r.slug==='kleopatra'?<span>Központ</span>:<div style={{minWidth:130}}><div style={{display:'flex',justifyContent:'space-between',gap:8,fontSize:12}}><b>{progress}%</b><span>{r.onboarding_ready?'Üzemkész':'Folyamatban'}</span></div><div style={{height:7,background:'#e5e7eb',borderRadius:999,overflow:'hidden',marginTop:5}}><div style={{height:'100%',width:`${progress}%`,background:'currentColor'}}/></div></div>}</td><td>{r.slug==='kleopatra'?'—':<><b>{next}</b>{r.admin_invitation_status==='pending'&&r.onboarding_next_step==='admin'?<><br/><small>aktiválásra vár</small></>:null}</>}</td><td>{r.slug==='kleopatra'?'—':<><span>{invite}</span>{r.admin_invitation_email?<><br/><small>{r.admin_invitation_email}</small></>:null}{r.admin_invitation_status==='pending'&&r.admin_invitation_expires_at?<><br/><small>Lejár: {new Date(r.admin_invitation_expires_at).toLocaleString('hu-HU')}</small></>:null}</>}</td><td>{Number(r.location_count||0)}</td><td>{Number(r.user_count||0)}</td><td>{r.slug==='kleopatra'?<span>Központi tenant</span>:<select value={r.status} disabled={saving} onChange={e=>void changeStatus(r,e.target.value)}><option value="trial">Trial</option><option value="active">Aktív</option><option value="suspended">Felfüggesztett</option><option value="cancelled">Megszüntetett</option></select>}</td></tr>}) : <tr><td colSpan={9}>{loading?'Tenantok betöltése…':'Nincs megjeleníthető tenant.'}</td></tr>}</tbody></table></div>
  </section>
 </div>;
}
