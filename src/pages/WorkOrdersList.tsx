...
export default function WorkOrdersList(){
  ...
  useEffect(()=>{void load()},[]);
  const canEdit=Boolean(scope?.can_edit??items.some(x=>x.can_edit));
  const scopeText=useMemo(()=>scope?.kind==='all'?'Összes szalon munkalapjai':scope?.kind==='location'?'A saját szalon munkalapjai':scope?.kind==='employee'?'Saját munkalapjaim':scope?.kind==='customer'?'Saját ügyfélmunkalapjaim':'Munkalapok',[scope]);
