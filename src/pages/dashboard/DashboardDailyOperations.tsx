...
export default function DashboardDailyOperations(){
 ...
 useEffect(()=>{let active=true;(async()=>{setLoading(true);setError("");try{const date=today();const locationId=String(user?.location_id||"");const params:any={from:date,to:date};if(locationId)params.location_id=locationId;const[e,a,s,t]=await Promise.all([api.get("/employees",{params:locationId?{location_id:locationId}:{}}),api.get("/timetable",{params}),api.get("/timetable/schedule",{params}),getVirTargets(locationId||undefined)]);if(!active)return;const employees=asArray<Employee>(e.data).filter(x=>!locationId||!x.location_id||String(x.location_id)===locationId);const raw=Array.isArray(a.data?.appointments)?a.data.appointments:asArray<BookingOperationAppointment>(a.data);setEmployeeCount(employees.length);setAppointments(raw);setScheduledMinutes(Number.isFinite(Number(s.data?.summary?.scheduled_minutes))?Number(s.data.summary.scheduled_minutes):null);setDailyTarget(Number.isFinite(Number(t.daily_revenue))?Number(t.daily_revenue):null)}catch(reason:any){if(active)setError(reason?.response?.data?.error||reason?.message||"A napi működési áttekintés nem tölthető be.")}finally{if(active)setLoading(false)}})();return()=>{active=false}},[user?.location_id]);
