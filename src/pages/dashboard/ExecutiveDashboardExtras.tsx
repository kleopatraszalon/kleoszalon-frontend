...
export default function ExecutiveDashboardExtras({ stats, alerts = [] }: Props) {
  ...
  const summary = useMemo(() => {
    ...
    const riskText = alerts.length ? `${alerts.length} vezetői figyelmeztetés vár ellenőrzésre.` : "Jelenleg nincs kiemelt vezetői kockázat.";
    return {
  ...
  return <section className="executive-extras">
    <article className="executive-summary-card"><div className="executive-summary-card__icon"><Sparkles aria-hidden="true" /></div><div className="executive-summary-card__content"><span>VEZETŐI ÖSSZEFOGLALÓ</span><h2>{summary.headline}</h2><p>{summary.body}</p></div>{alerts.length > 0 && <div className="executive-summary-card__risk"><AlertTriangle aria-hidden="true" /><strong>{alerts.length}</strong><small>figyelmeztetés</small></div>}</article>
