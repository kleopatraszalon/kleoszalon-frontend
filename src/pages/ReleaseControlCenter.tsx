...
const order=['Verzió és build','Runtime','Adatbázis','Automatikus tesztek','Üzleti folyamatok','Integrációk','Biztonság','Infrastruktúra','Üzemeltetés','Kódstabilitás','Jóváhagyás'];
...
const previousBusinessDate=()=>new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Budapest',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date(Date.now()-86400000));

function withProcessIntegrityGate(raw:Snapshot,process:any):Snapshot{
 const processStatus:GateStatus=process?.status==='ok'?'pass':process?.status==='warning'?'warning':'fail';
 const labels=(process?.processes||[]).filter((x:any)=>x.status!=='ok').map((x:any)=>`${x.key}: ${x.exception_count} kivétel`).join('; ');
 const gate:Gate={key:'business.process_integrity',group:'Üzleti folyamatok',label:'Előző üzleti nap teljes folyamatintegritása',status:processStatus,blocking:true,message:processStatus==='pass'?`${process?.business_date||previousBusinessDate()}: a pénzügyi, készlet-, beszerzési és rendszer-invariáns kontrollok zöldek.`:`${process?.business_date||previousBusinessDate()}: ${process?.exception_count||0} folyamatkivétel. ${labels||'A teljes üzleti lánc nem zárt hibamentesen.'}`,evidence:process?.generated_at||null,source:'business-process-integrity'};
 const gates=[...(raw.gates||[]).filter(g=>g.key!==gate.key),gate];
 const processBlocks=gate.status!=='pass';
 const blockers=[...(raw.blockers||[]).filter(b=>b.key!==gate.key),...(processBlocks?[{key:gate.key,label:gate.label,status:gate.status,message:gate.message}]:[])];
 const summary={...raw.summary,total:gates.length,pass:gates.filter(g=>g.status==='pass').length,warning:gates.filter(g=>g.status==='warning').length,fail:gates.filter(g=>g.status==='fail').length,pending:gates.filter(g=>g.status==='pending').length,blocking_total:gates.filter(g=>g.blocking).length,blocking_open:gates.filter(g=>g.blocking&&g.status!=='pass').length};
 const releaseReady=raw.release_ready&&!processBlocks;
 return{...raw,gates,blockers,summary,release_ready:releaseReady,decision:releaseReady?'GO':'NO-GO',meta:{...raw.meta,process_integrity_business_date:process?.business_date||null,process_integrity_status:process?.status||'unknown',process_integrity_exception_count:process?.exception_count??null}};
}
...
export default function ReleaseControlCenter(){
 ...
 const load=useCallback(async()=>{setLoading(true);setError('');try{const q=new URLSearchParams({date:previousBusinessDate()});const locationId=localStorage.getItem('kleo_location_id')||'';if(locationId)q.set('location_id',locationId);const[r,p]=await Promise.all([api.get('/api/transactions/release-control'),api.get(`/api/transactions/notifications/reconciliation/process-integrity?${q}`)]);const merged=withProcessIntegrityGate(r.data,p.data);setData(merged);const next:Record<string,{status:GateStatus;evidence:string}>={};for(const g of(merged.gates||[]))if(g.editable)next[g.key]={status:g.status,evidence:g.evidence||''};setDrafts(next)}catch(e:any){setError(e?.response?.data?.message||'A Release Control Center vagy a folyamatintegritási gate nem tölthető be.')}finally{setLoading(false)}},[]);
 ...
 return <section className="release-control-center">
  <div className="release-control-hero">
   <div className="release-control-title"><span>ADMINISZTRÁCIÓ · RELEASE GATE</span><h1><ShieldCheck/> Release Control Center</h1><p>Élesítési döntés egy helyen: verzió, adatbázis, tesztek, üzleti folyamatintegritás, integrációk, infrastruktúra és jóváhagyási bizonyítékok.</p></div>
  ...
  {data&&<>
   <div className="release-meta-grid">
    ...
    <article><Server/><div><small>FOLYAMATINTEGRITÁS</small><b>{String(data.meta?.process_integrity_status||'unknown').toUpperCase()}</b><span>{data.meta?.process_integrity_exception_count??'—'} kivétel · {data.meta?.process_integrity_business_date||'nincs nap'}</span></div></article>
   ...
   <div className={`release-final ${data.release_ready?'go':'no-go'}`}>{data.release_ready?<><CheckCircle2/><div><b>GO – a kötelező release gate-ek és az üzleti folyamatok teljesülnek</b><span>A kiadás technikailag, bizonyíték és előző napi üzleti integritás szinten engedélyezhető. A production deploy továbbra is a jóváhagyott release-folyamaton keresztül történjen.</span></div></>:<><AlertTriangle/><div><b>NO-GO – az élesítés jelenleg blokkolt</b><span>A kötelező kapuk mellett az előző üzleti nap folyamatintegritásának is PASS állapotúnak kell lennie; figyelmeztetés vagy kritikus eltérés mellett nincs GO.</span></div></>}</div>
  </>}
 </section>
}
