...
test("admin login and app version notifications are supported",()=>{expect(source).toContain('"customer","admin"');expect(source).toContain("Admin felület");expect(source).toContain("app-version.json");expect(source).toContain("showNotification");expect(source).toContain('APP_VERSION="1.1.0"')});
