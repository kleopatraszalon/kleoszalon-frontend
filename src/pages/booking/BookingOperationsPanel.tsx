...
export default function BookingOperationsPanel({ appointments, employeeCount, onOpenAppointment }: Props) {
  ...
  useEffect(()=>{
    if(!appointmentKey||!canEditBooking||ensureInFlight.current||lastEnsuredKey.current===appointmentKey)return;
    const pendingIds=appointments.filter(item=>!item.work_order_id).map(item=>item.id);
    if(!pendingIds.length){lastEnsuredKey.current=appointmentKey;return}
    let active=true;
    ...
    void(async()=>{
      try{
        ...
        lastEnsuredKey.current=appointmentKey;
      }catch(e:any){
        if(!active)return;
    ...
    return()=>{active=false};
  },[appointmentKey,canEditBooking,appointments]);
