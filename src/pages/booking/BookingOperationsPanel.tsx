...
export default function BookingOperationsPanel({ appointments, employeeCount, onOpenAppointment }: Props) {
  ...
  const arriveAndOpen=async(item:BookingOperationAppointment)=>{
    ...
    try{
      if(normalizeStatus(item.status)!=="arrived"&&normalizeStatus(item.status)!=="in_progress"){
        await api.patch(`/appointments/${item.id}`,{status:"arrived"});
      }
  ...
  return <section className="booking-operations" aria-label="Napi foglalási irányítóközpont">
    ...
    <div className="booking-operations__content"><article className="booking-operations__upcoming"><header><div><span>KÖVETKEZŐ VENDÉGEK</span><h3>Aktuális sorrend</h3></div><b>{summary.active.length} aktív</b></header>{summary.upcoming.length?summary.upcoming.map(item=><div key={item.id} className="booking-operations__arrivalrow" style={{display:"grid",gridTemplateColumns:"80px minmax(0,1fr) auto auto",gap:8,alignItems:"center",padding:"8px 0",borderBottom:"1px solid rgba(0,0,0,.06)"}}><time>{timeText(item.start_time)}</time><button type="button" onClick={()=>onOpenAppointment?.(item.id)} style={{border:0,background:"transparent",textAlign:"left",padding:0,cursor:"pointer"}}><b>{item.client_name||item.title||"Vendég"}</b><small style={{display:"block"}}>{normalizeStatus(item.status)} · {Math.round(minutesBetween(item.start_time,item.end_time))} perc</small></button><button type="button" disabled={arrivalBusy===item.id||["completed","cancelled","canceled","no_show"].includes(normalizeStatus(item.status))} onClick={()=>void arriveAndOpen(item)} style={{border:"1px solid #d6d0dc",borderRadius:10,padding:"7px 10px",background:"#fff",fontWeight:700,cursor:"pointer"}}>{arrivalBusy===item.id?"Nyitás…":normalizeStatus(item.status)==="arrived"||normalizeStatus(item.status)==="in_progress"?"Munkalap":"Megérkezett"}</button></div>):<p className="booking-operations__empty">Nincs további aktív foglalás.</p>}</article>
