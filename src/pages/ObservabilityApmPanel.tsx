...
export default function ObservabilityApmPanel(){
 const[snapshot,setSnapshot]=useState<Snapshot|null>(null),[history,setHistory]=useState<any[]>([]),[alerts,setAlerts]=useState<AlertRow[]>([]),[deliveries,setDeliveries]=useState<any[]>([]),[loading,setLoading]=useState(false),[error,setError]=useState(''),[running,setRunning]=useState(false);
 ...
 const chart=useMemo(()=>history.map((row:any)=>{const p=row.payload||{};return{time:new Date(row.captured_at).toLocaleTimeString('hu-HU',{hour:'2-digit',minute:'2-digit'}),p50:Number(p.api?.p50_ms||0),p95:Number(p.api?.p95_ms||0),p99:Number(p.api?.p99_ms||0),rate5xx:Number(p.api?.rate_5xx||0)}}),[history]);
