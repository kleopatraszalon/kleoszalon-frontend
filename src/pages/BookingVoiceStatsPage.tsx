...
type Stats={
 ...
 summary:{voice_requests:number;recognized:number;recognition_rate:number;ai_used:number;ai_share:number;voice_book_intents:number;voice_booking_conversions?:number;voice_bookings:number;voice_booking_records?:number;correlated_voice_booking_records?:number;conversion_rate:number;legacy_aggregate_conversion_rate?:number;conversion_tracking_coverage?:number;online_bookings:number;voice_booking_share:number;voice_waitlist:number;correlated_voice_waitlist?:number;voice_waitlist_conversions?:number;online_waitlist:number;avg_transcript_length:number};
 ...
 locations:Array<{location_id:string;name:string;voice_requests:number;recognized:number;exact_book_conversions?:number;voice_bookings:number;online_bookings:number;recognition_rate:number;conversion_rate:number;conversion_tracking_coverage?:number}>;
 daily:Array<{date:string;voice_requests:number;recognized:number;ai_used:number;exact_book_conversions?:number;voice_bookings:number;online_bookings:number}>;
 top_services:Array<{service_id:string;name:string;requests:number}>;
 recent:Array<{id:string;created_at:string;intent:string;recognized:boolean;ai_used:boolean;transcript_length:number;requested_date?:string|null;requested_time?:string|null;preferred_period?:string|null;missing_fields:string[];location_name?:string|null;employee_name?:string|null;service_count:number;converted_to_booking?:boolean;appointment_id?:string|null;booking_status?:string|null;converted_to_waitlist?:boolean;waitlist_id?:string|null}>;
 tracking?:{mode:string;historical_uncorrelated_records:number};
...
const outcomeLabel=(x:Stats["recent"][number])=>x.converted_to_booking?`Foglalás${x.booking_status?` · ${x.booking_status}`:""}`:x.converted_to_waitlist?"Várólista":"—";
...
export default function BookingVoiceStatsPage(){
 ...
 const exactConversions=totals?.voice_booking_conversions??totals?.voice_bookings??0;
 const trackingCoverage=totals?.conversion_tracking_coverage??0;

 return <main className="voice-stats-page">
  <header className="voice-stats-hero">
   <div><span>FOGLALÁS 3.0 · VOICE BOOKING</span><h1>Voice Booking statisztika</h1><p>Hangalapú foglalási kérések felismerése, AI-használata és pontos, eseményazonosítóval követett konverziója.</p></div>
  ...
  {!data&&loading?<div className="voice-stats-loading"><RefreshCw className="spin"/>Statisztika betöltése…</div>:data&&<>
   <section className="voice-kpis">
    ...
    <article><TrendingUp/><div><small>Pontos Voice konverzió</small><strong>{pct(totals?.conversion_rate)}</strong><span>{num(exactConversions)} eseményből tényleges foglalás · követési lefedettség {pct(trackingCoverage)}</span></div></article>
    <article><Activity/><div><small>Voice részarány</small><strong>{pct(totals?.voice_booking_share)}</strong><span>{num(totals?.voice_bookings)} aktív voice · {num(totals?.online_bookings)} normál online</span></div></article>
    <article><Sparkles/><div><small>Voice várólista</small><strong>{num(totals?.voice_waitlist)}</strong><span>korrelált: {num(totals?.correlated_voice_waitlist)} · normál online: {num(totals?.online_waitlist)}</span></div></article>
   </section>

   <section className="voice-stats-grid">
    <article className="voice-panel voice-panel-wide"><header><div><span>NAPI TREND</span><h2>Hangalapú kérés → pontos konverzió</h2></div></header><div className="voice-chart"><ResponsiveContainer width="100%" height={300}><LineChart data={data.daily}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date" tickFormatter={v=>String(v).slice(5)}/><YAxis allowDecimals={false}/><Tooltip/><Legend/><Line type="monotone" dataKey="voice_requests" name="Hangértelmezés" stroke="#b69861" strokeWidth={2}/><Line type="monotone" dataKey="recognized" name="Felismert" stroke="#a44574" strokeWidth={2}/><Line type="monotone" dataKey="exact_book_conversions" name="Pontos Voice konverzió" stroke="#2d251f" strokeWidth={2}/></LineChart></ResponsiveContainer></div></article>
    <article className="voice-panel"><header><div><span>AI KÖLTSÉG ÉS TOKEN</span><h2>AI-használat</h2></div></header><div className="voice-ai-box"><BrainCircuit/><strong>{num(data.ai.calls)} hívás</strong><span>{num(data.ai.input_tokens)} input token</span><span>{num(data.ai.output_tokens)} output token</span><b>${aiCost} becsült költség</b></div><p className="voice-privacy">A statisztikai API nem adja vissza a vendégek felismert szövegét. Transcript tárolás: <b>{data.privacy.transcripts_stored?"bekapcsolva":"kikapcsolva"}</b>.</p>{data.tracking&&<p className="voice-privacy">Konverziókövetés: <b>exact voice_event_id</b>. Korábbi, nem korrelált voice rekord: <b>{num(data.tracking.historical_uncorrelated_records)}</b>.</p>}</article>
   </section>

   <section className="voice-stats-grid">
    <article className="voice-panel voice-panel-wide"><header><div><span>SZALONOK</span><h2>Voice vs. normál online foglalás</h2></div></header><div className="voice-chart"><ResponsiveContainer width="100%" height={320}><BarChart data={data.locations}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name" interval={0} angle={-20} textAnchor="end" height={90}/><YAxis allowDecimals={false}/><Tooltip/><Legend/><Bar dataKey="voice_bookings" name="Aktív Voice foglalás" fill="#a44574"/><Bar dataKey="online_bookings" name="Normál online" fill="#b69861"/></BarChart></ResponsiveContainer></div></article>
   ...
   <section className="voice-stats-grid">
    ...
    <article className="voice-panel voice-panel-wide"><header><div><span>LEGUTÓBBI ESEMÉNYEK</span><h2>Voice felismerések és végső kimenet</h2></div></header><div className="voice-table-wrap"><table><thead><tr><th>Időpont</th><th>Szalon</th><th>Szándék</th><th>Állapot</th><th>Kimenet</th><th>AI</th><th>Szolg.</th><th>Hiányzó adat</th></tr></thead><tbody>{data.recent.map(x=><tr key={x.id}><td>{new Date(x.created_at).toLocaleString("hu-HU",{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"})}</td><td>{x.location_name||"—"}</td><td>{intentLabel(x.intent)}</td><td><span className={x.recognized?"ok":"warn"}>{x.recognized?"Felismert":"Hiányos"}</span></td><td><span className={x.converted_to_booking||x.converted_to_waitlist?"ok":""}>{outcomeLabel(x)}</span></td><td>{x.ai_used?"Igen":"Nem"}</td><td>{num(x.service_count)}</td><td>{x.missing_fields?.length?x.missing_fields.map(missingLabel).join(", "):"—"}</td></tr>)}</tbody></table></div></article>
   </section>
  </>}
 </main>
}
