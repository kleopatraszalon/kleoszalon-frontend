...
describe('Lifecycle policy admin panel',()=>{
 ...
 it('shows scheduler health and manual run control',()=>{expect(panel).toContain('/lifecycle-policy/health');expect(panel).toContain('run-now');expect(panel).toContain('Scheduler futtatása most');expect(panel).toContain('Scheduler futástörténet');});
