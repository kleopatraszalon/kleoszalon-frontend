...
import { getVirStaffDetail, VirStaffDetailResponse } from "../api/virDrilldown";
...
export default function VirStaffDetailPage() {
  const { staffId = "" } = useParams();
  const navigate = useNavigate();
  const [data, setData] = useState<VirStaffDetailResponse | null>(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    getVirStaffDetail(staffId).then(setData).catch((e: any) => setError(e?.message || "staff_detail_failed")).finally(() => setLoading(false));
  }, [staffId]);
  ...
  return (
    <div style={{ padding: 24, background: "#f6f7fb", minHeight: "100vh" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 18, gap: 12 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 28 }}>Dolgozó drill-down</h1>
          <div style={{ opacity: 0.7, marginTop: 6 }}>{data?.staff?.full_name || staffId}</div>
        </div>
      ...
      {data?.staff ? (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(180px, 1fr))", gap: 16, marginBottom: 20 }}>
            <div style={cardStyle}><div style={labelStyle}>Dolgozó</div><div style={valueStyle}>{data.staff.short_name || data.staff.full_name}</div></div>
            <div style={cardStyle}><div style={labelStyle}>Foglalás</div><div style={valueStyle}>{num(data.staff.appointments_count)}</div></div>
            <div style={cardStyle}><div style={labelStyle}>Árbevétel</div><div style={valueStyle}>{money(data.staff.revenue_total)}</div></div>
            <div style={cardStyle}><div style={labelStyle}>Árbevétel / óra</div><div style={valueStyle}>{money(data.staff.revenue_per_hour)}</div></div>
          </div>

          <div style={sectionStyle}>
            <div style={sectionTitle}>Szolgáltatás bontás</div>
            <table style={tableStyle}>
              <thead><tr><th style={thStyle}>Szolgáltatás</th><th style={thStyle}>Foglalás</th><th style={thStyle}>Árbevétel</th></tr></thead>
              <tbody>
                {data.services.map((row) => (
                  <tr key={row.service_id}>
                    <td style={tdStyle}>{row.service_name}</td>
