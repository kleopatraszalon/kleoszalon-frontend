...
export default function ProcurementWorkflowPanel({locationId,section="dashboard",onChanged}:Props){
 ...
 const canDashboardView=caps.menu("procurement.dashboard","can_view");
 const canDashboardEdit=caps.menu("procurement.dashboard","can_edit");
 const canApprovalsView=caps.menu("procurement.approvals","can_view");
 const canApprove=caps.menu("procurement.approvals","can_approve");
 const canExport=caps.menu("procurement.orders","can_export");
 const canPerformance=caps.menu("procurement.performance","can_view");
 const canDeviations=caps.menu("procurement.deviations","can_view");
 ...
 async function load(){
  setError("");
  try{
   const jobs:Promise<any>[]=[];const keys:string[]=[];
   if(canDashboardView){jobs.push(api.get("/api/transactions/procurement-workflow/settings"));keys.push("settings")}
   if(canApprovalsView){jobs.push(api.get("/api/transactions/procurement-workflow/pending"));keys.push("pending")}
   if(canPerformance){jobs.push(api.get("/api/transactions/procurement-workflow/supplier-performance"));keys.push("perf")}
   if(canDeviations){jobs.push(api.get("/api/transactions/procurement-workflow/alerts"));keys.push("alerts")}
   const result=await Promise.all(jobs);
   result.forEach((r,i)=>{switch(keys[i]){case"settings":setSettings(r.data);break;case"pending":setPending(arr<Pending>(r.data));break;case"perf":setPerf(arr<Perf>(r.data));break;case"alerts":setAlerts(arr<Alert>(r.data));break;}});
  }catch(e:any){setError(e?.response?.data?.error||e?.response?.data?.message||"A beszerzési kontrolladatok betöltése nem sikerült.")}
 }
 useEffect(()=>{if(!caps.loading)void load()},[locationId,caps.loading]); // eslint-disable-line react-hooks/exhaustive-deps
 async function saveSettings(){if(!canDashboardEdit)return;setSaving(true);setError("");try{const r=await api.put("/api/transactions/procurement-workflow/settings",{approval_threshold:Number(settings.approval_threshold),price_variance_warning_pct:Number(settings.price_variance_warning_pct)});setSettings(r.data);setSuccess("A jóváhagyási szabályok frissültek.");}catch(e:any){setError(e?.response?.data?.error||e?.response?.data?.message||"A beállítások mentése nem sikerült.")}finally{setSaving(false)}}
 async function approve(id:string|number){if(!canApprove)return;try{await api.post(`/api/transactions/procurement-workflow/orders/${id}/approve`,{});setSuccess(`A #${id} rendelés jóváhagyva.`);await load();onChanged?.()}catch(e:any){setError(e?.response?.data?.error||e?.response?.data?.message||"A jóváhagyás nem sikerült.")}}
 async function reject(id:string|number){if(!canApprove)return;const reason=window.prompt("Elutasítás indoka:");if(!reason?.trim())return;try{await api.post(`/api/transactions/procurement-workflow/orders/${id}/reject`,{reason:reason.trim()});setSuccess(`A #${id} rendelés elutasítva.`);await load();onChanged?.()}catch(e:any){setError(e?.response?.data?.error||e?.response?.data?.message||"Az elutasítás nem sikerült.")}}
 async function pdf(id:string|number){if(!canExport)return;try{const r=await api.get(`/api/transactions/procurement-workflow/orders/${id}/document.pdf`,{responseType:"blob"});const url=URL.createObjectURL(r.data);window.open(url,"_blank","noopener,noreferrer");setTimeout(()=>URL.revokeObjectURL(url),60000)}catch(e:any){setError(e?.response?.data?.error||e?.response?.data?.message||"A PDF bizonylat nem készíthető el.")}}

 const showDashboard=section==="dashboard"&&canDashboardView;
 const showApprovals=section==="approvals"&&canApprovalsView;
 const showPerformance=section==="performance"&&canPerformance;
 const showDeviations=section==="deviations"&&canDeviations;
 if(caps.loading)return <section className="pw-wrap"><div className="pw-empty">Jogosultságok betöltése…</div></section>;
 ...
 return <section className="pw-wrap">
  ...
  {showDashboard&&<>
   <div className="pw-settings"><label>Vezetői jóváhagyási értékhatár (Ft)<input disabled={!canDashboardEdit} type="number" min="0" value={settings.approval_threshold} onChange={e=>setSettings({...settings,approval_threshold:e.target.value})}/></label><label>Áreltérés riasztási küszöb (%)<input disabled={!canDashboardEdit} type="number" min="0" step="0.1" value={settings.price_variance_warning_pct} onChange={e=>setSettings({...settings,price_variance_warning_pct:e.target.value})}/></label>{canDashboardEdit&&<button onClick={saveSettings} disabled={saving}>{saving?"Mentés…":"Szabályok mentése"}</button>}</div>
