...
type Suggestion = {
  ...
  suggested_quantity: number|string; expected_cost: number|string; supplier_id?:number|null; supplier_name?:string|null; minimum_order_quantity?:number|string|null; lead_time_days?:number|null; preferred?:boolean;
};
type Order = {
  id: string|number; location_id?:string|null; supplier_id?:number|null; supplier_name:string; supplier_display_name?:string; status:string; expected_at?:string|null; note?:string|null;
...
export default function ProcurementPanel({locationId,onInventoryChanged}:Props){
  ...
  return <div className="procurement">
    ...
    <div className="proc-grid">
      <section className="proc-card"><header><div><span>MANUÁLIS UTÁNRENDELÉS</span><h2>Beszerzési javaslatok</h2><p>A preferált beszállítói ár és MOQ már megjelenik; a fenti intelligencia blokk automatikusan beszállítónként is csoportosít.</p></div><button onClick={load}>Frissítés</button></header>
        <div className="proc-orderbar"><input placeholder="Beszállító neve (kézi rendeléshez)" value={supplier} onChange={e=>setSupplier(e.target.value)}/><input type="date" value={expectedAt} onChange={e=>setExpectedAt(e.target.value)}/><strong>{selectedSuggestions.length} tétel · {huf(selectedCost)}</strong><button onClick={createOrder}>Kézi rendelés</button></div>
        <div className="proc-table"><table><thead><tr><th></th><th>Termék</th><th>Beszállító</th><th>Készlet</th><th>Minimum</th><th>MOQ</th><th>Javasolt</th><th>Egységár</th><th>Lead time</th><th>Költség</th></tr></thead><tbody>{suggestions.map(s=><tr key={String(s.balance_id)}><td><input type="checkbox" checked={!!selected[String(s.balance_id)]} onChange={e=>setSelected(x=>({...x,[String(s.balance_id)]:e.target.checked}))}/></td><td><b>{s.product_name}</b><small>{[s.brand,s.internal_code].filter(Boolean).join(" · ")}</small></td><td>{s.supplier_name||"—"}{s.preferred&&<small>Preferált</small>}</td><td>{qty(s.current_quantity)}</td><td>{qty(s.min_quantity)}</td><td>{s.minimum_order_quantity?qty(s.minimum_order_quantity):"—"}</td><td><b>{qty(s.suggested_quantity)}</b></td><td>{huf(s.unit_cost)}</td><td>{s.lead_time_days==null?"—":`${s.lead_time_days} nap`}</td><td><b>{huf(s.expected_cost)}</b></td></tr>)}{!loading&&!suggestions.length&&<tr><td colSpan={10} className="proc-empty">Nincs minimum alatti készlet.</td></tr>}</tbody></table></div>
      </section>
      <section className="proc-card"><header><div><span>BESZERZÉSI FOLYAMAT</span><h2>Rendelések</h2><p>Piszkozat → megrendelve → részben/teljesen beérkezett.</p></div></header><div className="proc-table"><table><thead><tr><th>#</th><th>Beszállító</th><th>Státusz</th><th>Tétel</th><th>Várható érték</th><th>Várható érkezés</th><th></th></tr></thead><tbody>{orders.map(o=><tr key={String(o.id)}><td>#{o.id}</td><td><b>{o.supplier_display_name||o.supplier_name}</b></td><td><span className={`proc-status ${o.status}`}>{statusLabels[o.status]||o.status}</span></td><td>{o.item_count}</td><td>{huf(o.expected_total)}</td><td>{o.expected_at?new Date(o.expected_at).toLocaleDateString("hu-HU"):"—"}</td><td><button onClick={()=>open(o.id)}>Megnyitás</button></td></tr>)}{!loading&&!orders.length&&<tr><td colSpan={7} className="proc-empty">Még nincs beszerzési rendelés.</td></tr>}</tbody></table></div></section>
    </div>
