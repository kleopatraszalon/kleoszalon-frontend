...
export default function RoleDashboardPage(){const{user,loading}=useCurrentUser();if(loading)return <div style={{padding:28,display:"flex",gap:8,alignItems:"center"}}><RefreshCw className="spin"/> Betöltés…</div>;const r=roles(user?.role);const accounting=r.some(x=>["accounting","bookkeeper","konyveles","könyvelés"].includes(x));const customer=r.some(x=>["customer","client","guest","ugyfel","ügyfél","vendeg","vendég"].includes(x));const receptionist=r.some(x=>["receptionist","reception","recepciós","recepcios"].includes(x));const payrollEligible=r.some(x=>["admin","administrator","rendszergazda","superadmin","super_admin","location_manager","üzletvezető","uzletvezeto","store_manager","branch_manager"].includes(x));const elevated=r.some(x=>["admin","administrator","rendszergazda","superadmin","super_admin","manager","vezető","vezeto","location_manager","üzletvezető","uzletvezeto","store_manager","branch_manager","szalonvezető","szalonvezeto","salon_manager"].includes(x));const staff=!elevated&&!customer&&!receptionist&&!accounting&&r.some(x=>["employee","staff","munkatárs","munkatars","professional","specialist"].includes(x));if(accounting)return <AccountingDashboardPage/>;if(customer)return <WithWorkOrders><CustomerDashboardPage/></WithWorkOrders>;if(receptionist)return <ReceptionDashboardPage/>;if(staff)return <WithWorkOrders><EmployeeDashboardPage/></WithWorkOrders>;return <WithWorkOrders><>{payrollEligible&&<HrPayrollReadinessPanel/>}<DashboardDailyOperations/><Home/></></WithWorkOrders>}