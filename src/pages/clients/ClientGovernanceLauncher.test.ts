...
test("keeps existing CRM and adds governance as an additive layer",()=>{expect(wrapper).toContain("ClientsCRMCore");expect(wrapper).toContain("ClientGovernanceLauncher");for(const marker of ["Ügyféltörzs","Címkék és szegmensek","Import és duplikációk","Altegio XLSX import"])expect(core).toContain(marker)});
