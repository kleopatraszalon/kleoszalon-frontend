...
const Home: React.FC = () => {
  ...
  id: selectedEvent ? selectedEvent.id : String(Date.now()),   // vagy: crypto.randomUUID()
...
// 1) Segédfüggvény: normalizáljuk a rekordot egységes alakra
const normService = (s: any) => ({
  id: s.id ?? s.uuid ?? s.service_id,                    // UUID
...
// 2) Betöltéskor próbáljunk két lehetséges végpontot (ha az első 404/empty)
useEffect(() => {
  ...
  const fetchWithAuth = (url: string) =>
    fetch(url, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => {
        if (r.status === 401) { navigate("/login"); throw new Error("401"); }
        return r.json().catch(() => []);
      })
      .catch(() => []);
  ...
  (async () => {
    ...
    // 🔹 Services: próbáljuk /api/services, ha üres, próbáljuk /api/masters/services
    ...
    if (!Array.isArray(sv)) sv = [];

    setServices(sv.map(normService)); // <-- egységesítve kerül a modalba
  })();
}, [token, navigate]);
export default Home;
