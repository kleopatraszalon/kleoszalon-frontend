...
describe('VIR P8 communication revenue automation',()=>{
  ...
  it('preserves consent autonomy and attribution disclosures',()=>{expect(page).toContain('Consent + approval boundary');expect(page).toContain('automatic_send=false');expect(page).toContain('Nem állít kauzalitást');expect(page).toContain('nem azonos a fizetett ledger-revenue-val');});
});
