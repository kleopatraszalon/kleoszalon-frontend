...
describe('VIR P8 communication revenue automation',()=>{
 ...
 it('surfaces provider callbacks and paid-ledger attribution',()=>{expect(page).toContain('Consent + approval boundary');expect(page).toContain('automatic sending is disabled');expect(page).toContain('Delivery/read/click');expect(page).toContain('Fizetett bevétel');expect(page).toContain('Nem állít kauzalitást');});
});
