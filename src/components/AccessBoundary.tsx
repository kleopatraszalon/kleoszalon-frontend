...
function ruleFor(pathname: string, search: string): Rule | null {
  ...
  if (pathname === "/finance/nav-online-invoice") return { feature:"finance",menu:"finance.invoices",financial:true };
  ...
  if (pathname === "/products" || pathname === "/warehouse/products" || pathname.startsWith("/warehouse/products/")) return { feature: "inventory", menu: "inventory.products" };
  if (pathname === "/warehouse/operations") return { feature:"inventory",menu:"inventory",financial:true };
  ...
  if (pathname.startsWith("/reports")) return { feature:"reports" };
  if (pathname === "/" || pathname.startsWith("/dashboard")) return { feature: "management_dashboard" };
  if (pathname.startsWith("/audit") || pathname.includes("audit")) return { feature: "audit", menu: "settings.audit" };
  return null;
}

function accountingPathAllowed(pathname:string,search:string){
  const q=new URLSearchParams(search);
  if(["/","/dashboard","/dashboard/summary","/dashboard/quick"].includes(pathname))return true;
  if(pathname==="/finance"||pathname==="/penzugy"||pathname.startsWith("/finance/")||pathname.startsWith("/modules/finance/"))return true;
  if(pathname==="/modules/team/payroll")return true;
  if(pathname==="/warehouse"||pathname==="/warehouse/list"||pathname==="/warehouse/products"||pathname.startsWith("/warehouse/products/")||pathname==="/warehouse/operations"||pathname==="/logisztika")return true;
  if(pathname.startsWith("/inventory/"))return true;
  if(pathname==="/workorders"||pathname.startsWith("/workorders/"))return true;
  if(pathname==="/employees"||pathname.startsWith("/employees/"))return true;
  if(pathname.startsWith("/modules/customers/")||pathname==="/modules/clients"||pathname==="/modules/crm")return true;
  if(pathname.startsWith("/reports"))return true;
  if(pathname==="/knowledge-base"||pathname.startsWith("/knowledge-base/"))return true;
  if(pathname.startsWith("/masterdata/"))return ["/masterdata/suppliers","/masterdata/payment-methods","/masterdata/financial-transaction-types","/masterdata/warehouses","/masterdata/salons","/masterdata/assets","/masterdata/equipment-types"].includes(pathname);
  if(pathname.startsWith("/marketing/"))return true;
  if(pathname.startsWith("/audit")||pathname.includes("audit"))return true;
...
export default function AccessBoundary({ children }: { children: React.ReactNode }) {
  ...
  if(!featureAllowed||!menuAllowed||!financialAllowed)return <Denied title="Nincs hozzáférése ehhez a modulhoz" detail="A menüpont, funkció vagy érzékeny adat nincs engedélyezve az aktuális szerepkör számára."/>;
  return <>{children}</>;
}
