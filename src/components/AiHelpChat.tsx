...
type StaffMessage = { id: string | number; sender_key: string; sender_name?: string; content: string; created_at: string };
type UsageStats = {
...
export default function AiHelpChat({ pageTitle }: { pageTitle?: string }) {
  ...
  async function loadStats() {
    try {
      const { data } = await api.get("/api/transactions/ai-support/stats");
      setStats(data);
    } catch {}
  }
  ...
  async function loadStaffHome() {
    try {
      const [people, convs] = await Promise.all([
        api.get("/api/transactions/staff-chat/coworkers"),
        api.get("/api/transactions/staff-chat/conversations"),
      ]);
      setCoworkers(people.data || []);
      setConversations(convs.data || []);
    } catch (err: any) {
      setError(err?.response?.data?.message || "A munkatársi chat nem tölthető be.");
    }
  }
  ...
  async function startConversation(coworker: Coworker) {
    ...
    try {
      ...
      setConversationName(coworker.full_name);
      await loadStaffMessages(String(data.id));
      await loadStaffHome();
    } catch (err: any) {
      setError(err?.response?.data?.message || "A beszélgetés nem indítható.");
    } finally { setStaffLoading(false); }
  }

  async function openConversation(conv: Conversation) {
    setConversationId(String(conv.id));
    setConversationName(conv.other_name || "Munkatárs");
    await loadStaffMessages(String(conv.id));
  }

  async function loadStaffMessages(id: string) {
    try {
      const { data } = await api.get(`/api/transactions/staff-chat/conversations/${id}/messages`);
  ...
  async function sendStaffMessage(e?: FormEvent) {
    ...
    try {
      ...
      setStaffInput("");
      await loadStaffMessages(conversationId);
      await loadStaffHome();
    } catch (err: any) {
      setError(err?.response?.data?.message || "Az üzenet nem küldhető el.");
    } finally { setStaffLoading(false); }
  }

  const mySenderKey = localStorage.getItem("email") ? `email:${String(localStorage.getItem("email")).toLowerCase()}` : "";
  ...
  return (
    <div className={`ai-help ${open ? "is-open" : ""}`}>
        ...
        <section className="ai-help-panel" aria-label="Kleo kommunikációs központ">
          <header className="ai-help-header">
            ...
            <div className="ai-help-actions">
              {mode === "ai" && <button type="button" onClick={clearHistory} title="AI beszélgetés törlése"><Trash2 size={16}/></button>}
          ...
          {mode === "ai" ? (
            <>
              ...
              {showStats && stats && <div className="ai-usage-card">
                <div><b>{stats.mine.request_count}</b><span>saját kérés / hó</span></div>
                <div><b>${stats.mine.estimated_cost_usd.toFixed(3)}</b><span>saját költség</span></div>
                <div><b>${stats.global.estimated_cost_usd.toFixed(3)}</b><span>összes AI költség</span></div>
                <small>Limit: {stats.limits.userMonthlyRequestLimit} kérés · ${stats.limits.userMonthlyBudgetUsd}/fő · ${stats.limits.monthlyBudgetUsd}/hó</small>
              </div>}
              <div className="ai-help-messages">
                {messages.map((m, index) => <div key={`${m.role}-${index}`} className={`ai-help-message ${m.role}`}>{m.role === "assistant" && <Bot size={16}/>}<p>{m.content}</p></div>)}
              ...
              <form className="ai-help-form" onSubmit={sendMessage}>
                <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); if (canSend) sendMessage(); } }} placeholder="Kérdezz a rendszer használatáról…" rows={2} maxLength={4000}/>
            ...
            <>
              {!conversationId ? <div className="staff-chat-home">
                <h4>Beszélgetések</h4>
                {conversations.length > 0 && <div className="staff-conversation-list">{conversations.map(c => <button key={c.id} onClick={() => openConversation(c)}><b>{c.other_name}</b><span>{c.last_message || "Beszélgetés megnyitása"}</span></button>)}</div>}
                <h4>Új beszélgetés</h4>
                <div className="staff-coworker-list">{coworkers.map(c => <button key={c.id} onClick={() => startConversation(c)}><UserRound size={17}/><span><b>{c.full_name}</b><small>{c.email || "Munkatárs"}</small></span></button>)}</div>
                {staffLoading && <div className="staff-loading"><Loader2 className="ai-help-spin" size={17}/> Betöltés…</div>}
                {error && <div className="ai-help-error">{error}</div>}
              </div> : <>
                <div className="staff-chat-bar"><button onClick={() => { setConversationId(""); setConversationName(""); }}>‹ Vissza</button><b>{conversationName}</b></div>
                <div className="ai-help-messages staff-messages">
                  {staffMessages.map(m => {
                    const mine = mySenderKey && m.sender_key === mySenderKey;
                    return <div key={m.id} className={`ai-help-message ${mine ? "user" : "assistant"}`}><p><small>{m.sender_name}</small>{m.content}</p></div>;
                  })}
                  {error && <div className="ai-help-error">{error}</div>}<div ref={endRef}/>
                </div>
                <form className="ai-help-form" onSubmit={sendStaffMessage}>
                  <textarea value={staffInput} onChange={e => setStaffInput(e.target.value)} placeholder={`Üzenet ${conversationName} részére…`} rows={2} maxLength={4000}/>
                  <button type="submit" disabled={!staffInput.trim() || staffLoading}><Send size={18}/></button>
                </form>
      ...
      <button className="ai-help-launcher" type="button" onClick={() => setOpen(v => !v)} aria-label={open ? "Chat bezárása" : "Segítség és chat megnyitása"}>
        {open ? <X size={22}/> : <><MessageCircle size={23}/><span>Segítség & Chat</span></>}
      </button>
    </div>
  );
}
