...
export default function AiHelpChat({ pageTitle }: { pageTitle?: string }) {
  ...
  return <div className={`ai-help ${open ? "is-open" : ""}`}>
    ...
    <button className="ai-help-launcher" type="button" onClick={()=>setOpen(v=>!v)} aria-label={open?"Chat bezárása":"Segítség és chat megnyitása"}>{open?<X size={22}/>:<><MessageCircle size={23}/><span>Segítség & Chat</span>{unreadCount>0&&<em className="ai-help-launcher-badge">{unreadCount>99?"99+":unreadCount}</em>}</>}</button>
  </div>;
}
