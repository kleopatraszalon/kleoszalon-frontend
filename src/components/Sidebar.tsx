...
import { BookOpenText, Boxes, Building2, CalendarDays, ChartNoAxesCombined, ChevronDown, ChevronRight, Circle, ClipboardCheck, Database, Gift, Globe2, GripVertical, LayoutDashboard, Megaphone, MessageSquareText, MonitorSmartphone, PlugZap, Settings, ShoppingBag, UserCog, Users, WalletCards, type LucideIcon } from "lucide-react";
...
const MENU_CACHE_KEY = "kleo.menu.cache.v7";
...
const menuIcons:Record<string,LucideIcon>={LayoutDashboard,CalendarDays,Users,Gift,UserCog,WalletCards,Boxes,ChartNoAxesCombined,Building2,Megaphone,Globe2,ShoppingBag,MonitorSmartphone,PlugZap,Settings,ClipboardCheck,BookOpenText,Database,MessageSquareText};
...
function roleList(raw:unknown):string[]{if(Array.isArray(raw))return raw.map(String).map(x=>x.toLowerCase());try{const p=JSON.parse(String(raw||""));if(Array.isArray(p))return p.map(String).map(x=>x.toLowerCase());if(p!=null)return[String(p).toLowerCase()]}catch{}return String(raw||"").split(",").map(x=>x.replace(/[\[\]"]/g,"").trim().toLowerCase()).filter(Boolean)}
const CUSTOMER_MENU:MenuItem[]=[
{id:99101,name:"Irányítópult",icon:"LayoutDashboard",route:"/",children:[]},
{id:99102,name:"Bejelentkezés",icon:"CalendarDays",route:"/customer/booking",children:[]},
];
const STAFF_MENU:MenuItem[]=[
{id:99001,name:"Irányítópult",icon:"LayoutDashboard",route:"/",children:[]},
{id:99002,name:"Saját beosztás",icon:"CalendarDays",route:"/modules/team/timetable",children:[]},
{id:99003,name:"Check listák",icon:"ClipboardCheck",route:"/knowledge-base/checklists",children:[]},
{id:99004,name:"Munkatársi chat",icon:"MessageSquareText",route:"/staff/chat",children:[]},
];
const LOCATION_MANAGER_MENU:MenuItem[]=[
{id:99201,name:"Irányítópult",icon:"LayoutDashboard",route:"/",children:[]},
{id:99202,name:"Időpontok és beosztás",icon:"CalendarDays",children:[
{id:99221,name:"Időpontnaptár",route:"/appointments/calendar",children:[]},
{id:99222,name:"Munkaidő és beosztás",route:"/modules/team/timetable",children:[]},
{id:99223,name:"Jelenlét",route:"/modules/team/attendance",children:[]},
]},
{id:99203,name:"Ügyfelek és CRM",icon:"Users",children:[
{id:99231,name:"Ügyfelek",route:"/modules/customers/clients",children:[]},
{id:99232,name:"CRM",route:"/modules/customers/crm",children:[]},
]},
{id:99204,name:"Munkatársak",icon:"UserCog",route:"/employees",children:[]},
{id:99205,name:"Munkalapok",icon:"ClipboardCheck",route:"/workorders",children:[]},
{id:99206,name:"Raktár és készlet",icon:"Boxes",children:[
{id:99261,name:"Készlet",route:"/warehouse",children:[]},
{id:99262,name:"Termékek",route:"/warehouse/products",children:[]},
]},
{id:99207,name:"Beszerzés",icon:"ShoppingBag",route:"/warehouse?view=procurement&section=dashboard",children:[]},
{id:99208,name:"Check listák",icon:"BookOpenText",route:"/knowledge-base/checklists",children:[]},
];
...
const Sidebar:React.FC<SidebarProps>=({user})=>{const location=useLocation();const navigate=useNavigate();const roleKeys=useMemo(()=>roleList(user?.role),[user?.role]);const isCustomer=roleKeys.some(r=>["customer","client","guest","ugyfel","ügyfél","vendeg","vendég"].includes(r));const isAdmin=roleKeys.some(r=>["admin","administrator","rendszergazda","superadmin","super_admin"].includes(r));const isLocationManager=roleKeys.some(r=>["location_manager","üzletvezető","uzletvezeto","store_manager","branch_manager"].includes(r));const isManager=isLocationManager||roleKeys.some(r=>["manager","vezető","vezeto"].includes(r));const isStaff=!isCustomer&&!isAdmin&&!isManager&&roleKeys.some(r=>["employee","receptionist"].includes(r));const [menus,setMenus]=useState<MenuItem[]>(()=>{try{const raw=localStorage.getItem(MENU_CACHE_KEY);const parsed=raw?JSON.parse(raw):null;return Array.isArray(parsed)&&parsed.length?parsed:FALLBACK}catch{return FALLBACK}});const [openIds,setOpenIds]=useState<number[]>([]);const [draggedId,setDraggedId]=useState<number|null>(null);const [dragOverId,setDragOverId]=useState<number|null>(null);const [savingOrder,setSavingOrder]=useState(false);const visibleMenus=isCustomer?CUSTOMER_MENU:isLocationManager?LOCATION_MANAGER_MENU:isStaff?STAFF_MENU:menus;
async function loadMenus(){if(isStaff||isCustomer||isLocationManager)return;const token=localStorage.getItem("token")||localStorage.getItem("kleo_token");const config={withCredentials:true,headers:token?{Authorization:`Bearer ${token}`} : undefined};for(const endpoint of ["menus","menu"]){try{const res=await axios.get(`${API_BASE}/${endpoint}`,config);const data=Array.isArray(res.data)?res.data as RawMenuItem[]:[];const tree=buildMenuTree(data,user?.role||null);if(tree.length){setMenus(tree);localStorage.setItem(MENU_CACHE_KEY,JSON.stringify(tree));return}}catch(e){console.warn(`Menü API hiba: ${endpoint}`,e)}}setMenus(prev=>prev.length?prev:FALLBACK)}
useEffect(()=>{void loadMenus()},[user,isStaff,isCustomer,isLocationManager]);
const toggle=(id:number)=>setOpenIds(p=>p.includes(id)?p.filter(x=>x!==id):[...p,id]);
useEffect(()=>{const parents:number[]=[];function walk(items:MenuItem[],ps:number[]=[]){items.forEach(i=>{if(i.route&&routePath(i.route)===location.pathname)parents.push(...ps);if(i.children.length)walk(i.children,[...ps,i.id])})}walk(visibleMenus);if(parents.length)setOpenIds(p=>Array.from(new Set([...p,...parents])))},[isStaff,isCustomer,isLocationManager,menus,location.pathname]);
