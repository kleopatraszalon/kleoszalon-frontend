...
import{BookOpenText,Boxes,CalendarDays,ChartNoAxesCombined,ChevronDown,ChevronRight,Circle,ClipboardCheck,Database,Dumbbell,Gift,LayoutDashboard,MonitorPlay,ShoppingBag,UserCog,Users,WalletCards,type LucideIcon}from'lucide-react';
...
const icons:Record<string,LucideIcon>={LayoutDashboard,CalendarDays,Users,Gift,UserCog,WalletCards,Boxes,ChartNoAxesCombined,ShoppingBag,ClipboardCheck,BookOpenText,Database,MonitorPlay,Dumbbell};
...
const ACCOUNTING:MenuItem[]=[
 ...
 {id:99302,name:'Pénzügyek',icon:'WalletCards',children:[
  ...
  {id:99326,name:'Nyugta és NAV-adatszolgáltatás',route:'/finance/receipt-compliance',children:[]},
  ...
  {id:99325,name:'Bérszámfejtés és járulékok',route:'/modules/team/payroll',children:[]}
 ]},
 ...
 {id:99304,name:'Raktár és készlet',icon:'Boxes',children:[
  {id:99341,name:'Készletáttekintés',route:'/warehouse',children:[]},
  ...
  {id:99344,name:'Sarzsok és lejáratok (FEFO)',route:'/warehouse/lots',children:[]}
 ]},
 {id:99305,name:'Könyvelési adatok',icon:'Database',children:[
  {id:99351,name:'Lezárt munkalapok',route:'/workorders',children:[]},
  {id:99352,name:'Munkatársak és bérezési adatok',route:'/employees',children:[]},
  {id:99353,name:'Ügyfelek és vevők',route:'/modules/customers/clients',children:[]},
  {id:99354,name:'Beszállítói törzs',route:'/masterdata/suppliers',children:[]},
  {id:99355,name:'Telephelyek és költséghelyek',route:'/masterdata/salons',children:[]},
 ...
 {id:99307,name:'Könyvelési tudásbázis',icon:'BookOpenText',route:'/knowledge-base',children:[]}
];
...
const SPEC_PARITY:MenuItem={id:99401,name:'VIR megfelelőségi ellenőrzés',icon:'ClipboardCheck',route:'/admin/vir/spec-parity',children:[]};
const WALLBOARD:MenuItem={id:99402,name:'TV napi akciók (WallBoard)',icon:'MonitorPlay',route:'/marketing/wallboard',children:[]};
const SAAS_ADMIN:MenuItem={id:99403,name:'SaaS és franchise központ',icon:'Database',route:'/admin/saas',children:[]};
const VIR_ADMIN:MenuItem={id:99404,name:'VIR adminisztráció',icon:'UserCog',route:'/knowledge-base/library?tab=vir',children:[]};
...
function configuredName(item:MenuItem,config:VirCustomization|null){const r=norm(item.route),key=menuKey(item),labels=config?.labels||{};if(r==='/knowledge-base/checklists')return'Ellenőrzőlisták';if(r==='/knowledge-base/processes')return'Folyamatok és szabályzatok';if(r==='/knowledge-base/quiz')return'Munkaköri teszt';if(r==='/knowledge-base/library?tab=vir')return'VIR adminisztráció';if(key==='dashboard'&&labels.dashboard)return labels.dashboard;if(key==='employees'&&labels.employees)return labels.employees;if(key==='clients'&&labels.clients)return labels.clients;if(key==='knowledge'&&labels.knowledge&&r==='/knowledge-base')return labels.knowledge;return canonicalMenuLabel(item.name)}
function withVirAdminInSettings(items:MenuItem[],enabled:boolean):MenuItem[]{
 if(!enabled)return items;
 let found=false;
 const next=items.map(item=>{
  const r=norm(item.route),settings=r==='/settings'||r.startsWith('/modules/settings')||/beállítások|settings/i.test(item.name);
  if(!settings)return item;
  found=true;
  const children=item.children.filter(c=>norm(c.route)!==norm(VIR_ADMIN.route));
  const hasSettingsEntry=children.some(c=>norm(c.route)==='/settings');
  const direct=item.route&&r!=='#'&&!hasSettingsEntry?{id:994040,name:'Rendszerbeállítások',route:item.route,children:[]}:null;
  return{...item,children:[...(direct?[direct]:[]),...children,VIR_ADMIN]};
 });
 if(found)return next;
 return[...next,{id:99407,name:'Beállítások és adminisztráció',icon:'UserCog',children:[{id:994070,name:'Rendszerbeállítások',route:'/settings',children:[]},VIR_ADMIN]}];
}
...
export default function Sidebar({user}:SidebarProps){
 ...
 const visible=useMemo(()=>{const base=isCustomer?CUSTOMER:isAccounting?ACCOUNTING:isLocationScoped?LOCATION:isStaff?STAFF:menus;let combined=base;if(isAdmin){const extras=[FITNESS_GYONGYOS,RECEIPT_COMPLIANCE,WALLBOARD,SPEC_PARITY,SAAS_ADMIN].filter(extra=>!base.some(x=>x.route===extra.route));combined=extras.length?[...base,...extras]:base}else if(isManager){const extras=[RECEIPT_COMPLIANCE,WALLBOARD,SPEC_PARITY].filter(extra=>!base.some(x=>x.route===extra.route));combined=extras.length?[...base,...extras]:base}else if(isLocationScoped&&!isCustomer){const extras=[...(fitnessAllowed?[FITNESS_GYONGYOS]:[]),RECEIPT_COMPLIANCE].filter(extra=>!base.some(x=>x.route===extra.route));combined=extras.length?[...base,...extras]:base}return withVirAdminInSettings(configuredMenu(combined,vir),isAdmin||isManager)},[isCustomer,isAccounting,isLocationScoped,isStaff,isAdmin,isManager,menus,vir,fitnessAllowed]);
