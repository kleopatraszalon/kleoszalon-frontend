...
const BROWSER_API_CONFIG={withCredentials:true,headers:{'X-Requested-With':'XMLHttpRequest'}};
...
const ACCOUNTING:MenuItem[]=[
 ...
 {id:99302,name:'Pénzügyek',icon:'WalletCards',children:[
  ...
  {id:99326,name:'Nyugta / NAV adatszolgáltatás',route:'/finance/receipt-compliance',children:[]},
...
const VIR_ADMIN:MenuItem={id:99404,name:'VIR Admin',icon:'UserCog',route:'/knowledge-base/library?tab=vir',children:[]};
const RECEIPT_COMPLIANCE:MenuItem={id:99405,name:'Nyugta / NAV adatszolgáltatás',icon:'WalletCards',route:'/finance/receipt-compliance',children:[]};
const FITNESS_GYONGYOS:MenuItem={id:99406,name:'Fitnesz – Gyöngyös',icon:'Dumbbell',route:'/finance/fitness',children:[{id:994061,name:'Áttekintés és bérletek',route:'/finance/fitness',children:[]},{id:994062,name:'Öltözőszekrények – 20 rekesz',route:'/finance/fitness/lockers',children:[]}]};
...
function configuredName(item:MenuItem,config:VirCustomization|null){const r=norm(item.route),key=menuKey(item),labels=config?.labels||{};if(r==='/knowledge-base/library')return'Tudásanyagok';if(r==='/knowledge-base/checklists')return'Check listák';if(r==='/knowledge-base/processes')return'Folyamatok';if(r==='/knowledge-base/quiz')return'Munkaköri teszt';if(r==='/knowledge-base/library?tab=vir')return'VIR Admin';if(key==='dashboard'&&labels.dashboard)return labels.dashboard;if(key==='employees'&&labels.employees)return labels.employees;if(key==='clients'&&labels.clients)return labels.clients;if(key==='knowledge'&&labels.knowledge&&r==='/knowledge-base')return labels.knowledge;return item.name}
...
export default function Sidebar({user}:SidebarProps){
 ...
 useEffect(()=>{if(!isAdmin&&!isLocationScoped){setFitnessAllowed(false);return}let alive=true;void(async()=>{try{const r=await axios.get(`${API_BASE}/vir/fitness/access`,BROWSER_API_CONFIG);if(alive)setFitnessAllowed(Boolean(r.data?.allowed))}catch{if(alive)setFitnessAllowed(false)}})();return()=>{alive=false}},[isAdmin,isLocationScoped,user]);
 useEffect(()=>{if(isCustomer||isAccounting||isLocationScoped||isStaff)return;void(async()=>{try{const r=await axios.get(`${API_BASE}/menus`,BROWSER_API_CONFIG),tree=buildTree(Array.isArray(r.data)?r.data:[],user?.role||null);if(tree.length){setMenus(tree);localStorage.setItem(MENU_CACHE_KEY,JSON.stringify(tree));return}}catch{}try{const p=JSON.parse(localStorage.getItem(MENU_CACHE_KEY)||'null');if(Array.isArray(p)&&p.length)setMenus(p)}catch{}})()},[user,isCustomer,isAccounting,isLocationScoped,isStaff]);
