...
import{BookOpenText,Boxes,Building2,CalendarDays,ChartNoAxesCombined,ChevronDown,ChevronRight,Circle,ClipboardCheck,Database,Dumbbell,Gift,Globe2,GripVertical,LayoutDashboard,Megaphone,MonitorPlay,Settings,ShoppingBag,UserCog,Users,WalletCards,type LucideIcon}from'lucide-react';
import Logo from'../assets/kleo_logo.png';
import SidebarCalendar from'./SidebarCalendar';
import{useLanguage}from'../i18n/LanguageProvider';
import{canonicalMenuLabel,menuLabel}from'../utils/menuLabels';
...
const MENU_CACHE_KEY='kleo.menu.cache.v19';
interface RawMenuItem{id:number;code?:string|null;name:string;icon?:string|null;route?:string|null;parent_id?:number|null;required_role?:string|null;order_index?:number|null;submenus?:RawMenuItem[]}
interface MenuItem{id:number;code?:string;name:string;icon?:string;route?:string;children:MenuItem[]}
interface SidebarProps{user?:{role?:string|string[]|null}|null}
const icons:Record<string,LucideIcon>={LayoutDashboard,CalendarDays,Users,Gift,UserCog,WalletCards,Boxes,ChartNoAxesCombined,ShoppingBag,ClipboardCheck,BookOpenText,Database,MonitorPlay,Dumbbell,Building2,Globe2,Megaphone,Settings,GripVertical};
...
const ROOT_LABELS:Record<string,string>={dashboard:'Irányítópult',appointments:'Időpontok és beosztás',customers:'Ügyfelek és CRM',loyalty:'Törzsvásárlói program',team:'Munkatársak és HR',finance:'Pénzügyek',inventory:'Raktár és készlet',procurement:'Beszerzés',analytics:'Vezetői riportok',locations:'Telephelyek',marketing:'Marketing',online:'Online foglalás és alkalmazás',commerce:'Értékesítés és webshop',settings:'Beállítások és adminisztráció'};
...
const STAFF:MenuItem[]=[{id:99001,name:'Irányítópult',icon:'LayoutDashboard',route:'/',children:[]},{id:99002,name:'Saját beosztás',icon:'CalendarDays',route:'/modules/team/timetable',children:[]},{id:99005,name:'Saját munkalapok',icon:'ClipboardCheck',route:'/workorders',children:[]},{id:99003,name:'Ellenőrzőlisták',icon:'ClipboardCheck',route:'/knowledge-base/checklists',children:[]},{id:99006,name:'Tudásbázis',icon:'BookOpenText',route:'/knowledge-base/library',children:[]},{id:99007,name:'Munkaköri teszt',icon:'ClipboardCheck',route:'/knowledge-base/quiz',children:[]}];
const LOCATION:MenuItem[]=[{id:99201,name:'Irányítópult',icon:'LayoutDashboard',route:'/',children:[]},{id:99202,name:'Időpontok és beosztás',icon:'CalendarDays',children:[{id:99221,name:'Időpontnaptár',route:'/appointments/calendar',children:[]},{id:99224,name:'Munkalapok',route:'/workorders',children:[]}]},{id:99206,name:'Raktár és készlet',icon:'Boxes',children:[{id:99261,name:'Készlet',route:'/warehouse',children:[]},{id:99262,name:'Termékek',route:'/warehouse/products',children:[]},{id:99263,name:'Sarzsok és lejáratok (FEFO)',route:'/warehouse/lots',children:[]}]},{id:99207,name:'Beszerzés',icon:'ShoppingBag',route:'/warehouse?view=procurement&section=dashboard',children:[]},{id:99208,name:'Tudásbázis',icon:'BookOpenText',route:'/knowledge-base/library',children:[]},{id:99209,name:'Munkaköri teszt',icon:'ClipboardCheck',route:'/knowledge-base/quiz',children:[]}];
const ACCOUNTING:MenuItem[]=[
 {id:99301,name:'Irányítópult',icon:'LayoutDashboard',route:'/',children:[]},
 {id:99302,name:'Pénzügyek',icon:'WalletCards',children:[{id:99321,name:'Pénzügyi áttekintés',route:'/finance',children:[]},{id:99322,name:'NAV Online Számla',route:'/finance/nav-online-invoice',children:[]},{id:99326,name:'Nyugta és NAV-adatszolgáltatás',route:'/finance/receipt-compliance',children:[]},{id:99323,name:'Számlák és bizonylatok',route:'/finance?section=invoices',children:[]},{id:99324,name:'Pénzügyi ellenőrzés',route:'/finance?section=control',children:[]},{id:99325,name:'Bérszámfejtés és járulékok',route:'/modules/team/payroll',children:[]}]},
 {id:99303,name:'Beszerzés',icon:'ShoppingBag',children:[{id:99331,name:'Beszerzési áttekintés',route:'/warehouse?view=procurement&section=dashboard',children:[]},{id:99332,name:'Jóváhagyások',route:'/warehouse?view=procurement&section=approvals',children:[]},{id:99333,name:'Rendelések',route:'/warehouse?view=procurement&section=orders',children:[]},{id:99334,name:'Beszállítók',route:'/warehouse?view=procurement&section=suppliers',children:[]}]},
 {id:99304,name:'Raktár és készlet',icon:'Boxes',children:[{id:99341,name:'Készletáttekintés',route:'/warehouse',children:[]},{id:99342,name:'Termékek',route:'/warehouse/products',children:[]},{id:99343,name:'Készletműveletek',route:'/warehouse/operations',children:[]},{id:99344,name:'Sarzsok és lejáratok (FEFO)',route:'/warehouse/lots',children:[]}]},
...
const MANAGEMENT_TOOLS:MenuItem={id:99407,name:'Vállalat irányítási eszközök',icon:'ChartNoAxesCombined',children:[
 {id:994071,name:'Áttekintés',route:'/reports/management-tools',children:[]},
 {id:994072,name:'Pareto elemzés',route:'/reports/management-tools?tool=pareto',children:[]},
 {id:994073,name:'SWOT analízis',route:'/reports/management-tools?tool=swot',children:[]},
 {id:994074,name:'Okfeltárás',route:'/reports/management-tools?tool=root-cause',children:[]},
 {id:994075,name:'Döntés és kockázat',route:'/reports/management-tools?tool=decision-risk',children:[]},
 {id:994076,name:'Folyamatfejlesztés',route:'/reports/management-tools?tool=improvement',children:[]},
 {id:994077,name:'Adateszközök',route:'/reports/management-tools?tool=data-tools',children:[]}
]};
const SAAS_ADMIN_TOOLS:MenuItem={id:99408,code:'saas.admin',name:'SaaS admin',icon:'Database',children:[
 {id:994081,code:'saas.platform',name:'SaaS platform és franchise',route:'/admin/saas',children:[]},
 {id:994082,code:'saas.migration',name:'VIR Migrációs Központ v18',route:'/admin/saas?workspace=migration-center',children:[]}
]};
const CUSTOMER_INTELLIGENCE_CHILD:MenuItem={id:994091,code:'customers.intelligence',name:'Customer Intelligence / Next Best Action',route:'/modules/customers/intelligence',children:[]};
const CUSTOMER_INTELLIGENCE_ROOT:MenuItem={id:99409,code:'customers',name:'Ügyfelek és CRM',icon:'Users',children:[CUSTOMER_INTELLIGENCE_CHILD]};

function stripRoutes(items:MenuItem[],routes:Set<string>):MenuItem[]{return items.flatMap(item=>{const children=stripRoutes(item.children,routes);if(item.route&&routes.has(norm(item.route))){if(children.length)return[{...item,route:undefined,children}];return[]}return[{...item,children}]})}
function injectCustomerIntelligence(items:MenuItem[]):MenuItem[]{let found=false;const walk=(xs:MenuItem[]):MenuItem[]=>xs.map(item=>{const children=walk(item.children);if(item.code==='customers'){found=true;return{...item,children:[...children.filter(c=>norm(c.route)!==norm(CUSTOMER_INTELLIGENCE_CHILD.route)),CUSTOMER_INTELLIGENCE_CHILD]}}return{...item,children}});const out=walk(items);return found?out:[...out,CUSTOMER_INTELLIGENCE_ROOT]}
...
function configuredName(item:MenuItem,config:VirCustomization|null){const r=norm(item.route),labels=config?.labels||{};if(item.code&&ROOT_LABELS[item.code]){const custom=item.code==='dashboard'?labels.dashboard:item.code==='team'?labels.employees:item.code==='customers'?labels.clients:undefined;return canonicalMenuLabel(custom||ROOT_LABELS[item.code])}if(r==='/knowledge-base/checklists')return'Ellenőrzőlisták';if(r==='/knowledge-base/processes')return'Folyamatok és szabályzatok';if(r==='/knowledge-base/quiz')return'Munkaköri teszt';if(r==='/knowledge-base/library?tab=vir')return'VIR adminisztráció';if(r==='/'&&labels.dashboard)return labels.dashboard;if(r==='/employees'&&labels.employees)return labels.employees;if((r==='/modules/customers/clients'||r==='/clients')&&labels.clients)return labels.clients;if(labels.knowledge&&r==='/knowledge-base')return labels.knowledge;return canonicalMenuLabel(item.name)}
...
export default function Sidebar({user}:SidebarProps){
 ...
 const[menus,setMenus]=useState<MenuItem[]>(FALLBACK),[open,setOpen]=useState<number[]>(()=>isAccounting?[99302,99303,99304]:[]),[vir,setVir]=useState<VirCustomization|null>(()=>readStoredVirCustomization()),[fitnessAllowed,setFitnessAllowed]=useState(false);
 ...
 useEffect(()=>{if(!isAdmin&&!isLocationScoped){setFitnessAllowed(false);return}let alive=true;void(async()=>{const token=localStorage.getItem('token')||localStorage.getItem('kleo_token'),config={withCredentials:true,headers:token?{Authorization:`Bearer ${token}`}:{}};try{const r=await axios.get(`${API_BASE}/vir/fitness/access`,config);if(alive)setFitnessAllowed(Boolean(r.data?.allowed))}catch{if(alive)setFitnessAllowed(false)}})();return()=>{alive=false}},[isAdmin,isLocationScoped,user]);
 ...
 const visible=useMemo(()=>{const base=isCustomer?CUSTOMER:isAccounting?ACCOUNTING:isLocationScoped?LOCATION:isStaff?STAFF:isAdmin?FALLBACK:menus;let combined=base;if(isLocationScoped&&!isCustomer){const extras=[...(fitnessAllowed?[FITNESS_GYONGYOS]:[]),RECEIPT_COMPLIANCE].filter(extra=>!base.some(x=>x.route===extra.route));combined=extras.length?[...base,...extras]:base}else if(isAdmin&&!fitnessAllowed){combined=base.filter(x=>x.code!=='fitness.gyongyos')}if((isAdmin||isManager)&&!combined.some(x=>x.id===MANAGEMENT_TOOLS.id))combined=[...combined,MANAGEMENT_TOOLS];if(isAdmin){combined=stripRoutes(combined,new Set(['/admin/saas','/admin/saas?workspace=migration-center']));combined=[...combined,SAAS_ADMIN_TOOLS]}if(isAdmin||isManager)combined=injectCustomerIntelligence(combined);return configuredMenu(combined,vir)},[isCustomer,isAccounting,isLocationScoped,isStaff,isAdmin,isManager,menus,vir,fitnessAllowed]);
