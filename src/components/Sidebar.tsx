...
export default function Sidebar({user}:SidebarProps){
 ...
 const visible=useMemo(()=>{const base=isCustomer?CUSTOMER:isAccounting?ACCOUNTING:isLocationScoped?LOCATION:isStaff?STAFF:menus;if(isAdmin){const extras=[WALLBOARD,SPEC_PARITY,SAAS_ADMIN].filter(extra=>!base.some(x=>x.route===extra.route));return extras.length?[...base,...extras]:base}if(isManager){const extras=[WALLBOARD,SPEC_PARITY].filter(extra=>!base.some(x=>x.route===extra.route));return extras.length?[...base,...extras]:base}return base},[isCustomer,isAccounting,isLocationScoped,isStaff,isAdmin,isManager,menus]);
