...
test('CAPA UI uses governed summary list sync detail and update API',()=>{const p=read('src/pages/ExceptionCapaPage.tsx');const detail='api.get(`'+dollar+'{BASE}/'+dollar+'{id}`)';const update='api.patch(`'+dollar+'{BASE}/'+dollar+'{selected.item.id}`';for(const marker of ['/summary','/sync',detail,update])expect(p).toContain(marker)});

test('only human-approved CAPA can create exactly one linked management improvement project',()=>{const p=read('src/pages/ExceptionCapaPage.tsx');const promote='api.post(`'+dollar+'{BASE}/'+dollar+'{id}/promote`';for(const marker of [promote,"String(selected.item.status)!=='approved'","String(current.status)==='approved'",'Fejlesztési projekt indítása','improvement_link','A projekt külön approval gate-en marad.','/operations/improvement?project='])expect(p).toContain(marker);expect(p).toContain("window.confirm('Létrehozzam a kapcsolt Fejlesztési projektet")});
