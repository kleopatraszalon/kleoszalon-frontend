...
test('approved CAPA can create exactly one linked management improvement project',()=>{const p=read('src/pages/ExceptionCapaPage.tsx');const promote='api.post(`'+dollar+'{BASE}/'+dollar+'{id}/promote`';for(const marker of [promote,"['approved','in_progress','verification','verified']",'Fejlesztési projekt indítása','improvement_link','A projekt külön approval gate-en marad.','/operations/improvement?project='])expect(p).toContain(marker);expect(p).toContain("window.confirm('Létrehozzam a kapcsolt Fejlesztési projektet")});
