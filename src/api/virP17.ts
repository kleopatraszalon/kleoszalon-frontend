...
export type P17OperationType='capacity_review'|'staffing_review'|'inventory_review'|'revenue_review'|'manual_task';
export type P17Status='pending_approval'|'approved'|'executed'|'verified'|'rolled_back'|'rejected';
export type P17Operation={id:string;tenant_id:string;location_id?:string|null;operation_type:P17OperationType;title:string;status:P17Status;risk_level:'low'|'medium'|'high'|'critical';created_at:string;updated_at:string};
...
export const createP17Operation=(body:{operation_type:P17OperationType;title:string;locationId?:string;idempotency_key?:string;preview_payload?:Record<string,unknown>})=>api.post('/vir/p17/operations',body).then(r=>r.data);
