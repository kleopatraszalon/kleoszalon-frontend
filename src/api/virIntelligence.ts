...
export type ProfitabilitySummary={revenue:number;material_cost:number;commission_cost:number;labor_cost:number;contribution_margin:number;margin_percent:number;closed_workorders:number;productive_hours:number};
export type ProfitabilityLocation=ProfitabilitySummary&{location_id:string|null;location_name:string|null};
export type ProfitabilityResponse={ok:boolean;from:string;to:string;summary:ProfitabilitySummary;by_location:ProfitabilityLocation[];cost_model:{includes_material_cost:boolean;includes_commission_cost:boolean;includes_direct_labor:boolean;monthly_hours_standard:number;excludes_employer_contributions_and_overhead:boolean}};
