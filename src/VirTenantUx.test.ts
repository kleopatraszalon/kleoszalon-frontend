...
describe('VIR searchable data selection and understandable UX',()=>{
 it('provides tenant scoped lookup client kinds through one generic endpoint',()=>{expect(api).toContain("'clients'|'locations'|'work-orders'");expect(api).toContain('/vir/lookups/');expect(api).toContain('kind');expect(api).toContain('/vir/lookups/context')});
