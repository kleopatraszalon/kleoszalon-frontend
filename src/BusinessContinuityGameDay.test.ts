...
test('GameDay UI uses management APIs with evidence based transitions',()=>{const p=read('src/pages/BusinessContinuityGameDayPage.tsx');for(const marker of ['/api/transactions/notifications/gameday','/templates','/service-readiness','/drills','/verification','/complete','/injects/','evidence('])expect(p).toContain(marker)});
