...
test('release control is fail-closed on previous-day process integrity',()=>{const p=read('src/pages/ReleaseControlCenter.tsx');expect(p).toContain('business.process_integrity');expect(p).toContain("blocking:true");expect(p).toContain("gate.status!=='pass'");expect(p).toContain("decision:releaseReady?'GO':'NO-GO'");expect(p).toContain('/api/transactions/notifications/reconciliation/process-integrity')});
