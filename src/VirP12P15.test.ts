...
describe('VIR P12-P15 management workspaces',()=>{
 ...
 it('exposes controlled P12-P15 capability wording in both UI languages',()=>{expect(p12).toContain('Automatikus profilátírás vagy megkeresés nincs.');expect(p12).toContain('without automatic profile changes or outreach');expect(p13).toContain('Terhelés és kedvezmény soha nem történik automatikusan.');expect(p14).toContain('A rendszer nem írja át automatikusan a beosztást.');expect(p15).toContain('nem hajtja végre automatikusan')});
 it('keeps searchable business selectors on customer-facing workflows',()=>{expect(p12).toContain('kind="clients"');expect(p13).toContain('kind="clients"');expect(p15).toContain('kind="locations"');});
 it('keeps the new modular route invariant',()=>expect(audit).toContain('routeMatches.length!==206'));
});
