...
test('transaction trace UI exposes lifecycle proof timeline and backfill',()=>{const p=read('src/pages/TransactionTracePage.tsx');for(const marker of ['TRANSACTION LIFECYCLE & TRACEABILITY','Append-only · SHA-256','Bizonyítható üzleti lánc','Proof of integrity','Eseménytimeline','Kapcsolt entitások','30 nap bizonyítási backfill','Hash-lánc újraellenőrzése'])expect(p).toContain(marker)});
