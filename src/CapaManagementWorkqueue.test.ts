...
test('CAPA management workqueue uses governed backend endpoints without hard-coded production host',()=>{const p=read('src/pages/CapaManagementWorkqueuePage.tsx');for(const marker of ['/api/transactions/notifications/exceptions/intelligence/capa/improvement-workqueue','/improvement-workqueue/assign','/improvement-workqueue/acknowledge','api.get','api.post'])expect(p).toContain(marker);expect(p).not.toContain('onrender.com')});
