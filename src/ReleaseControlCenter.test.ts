...
test('Release Control Center exposes strict server-side GO NO-GO state',()=>{for(const x of['Release Control Center','RELEASE READY','Élesítést blokkoló tételek','NO-GO','GO – a kötelező release gate-ek teljesülnek'])expect(page).toContain(x)});
