...
const routeFiles=['src/routing/publicRoutes.tsx','src/routing/bookingRoutes.tsx','src/routing/hrRoutes.tsx','src/routing/financeRoutes.tsx','src/routing/inventoryRoutes.tsx','src/routing/adminRoutes.tsx'];
...
const critical=['/appointments/calendar','/workorders','/modules/team/timetable','/modules/team/attendance','/modules/customers/clients','/modules/customers/crm','/employees','/loyalty','/finance','/warehouse','/warehouse/products','/warehouse/operations','/warehouse/lots','/warehouse/central-supply','/masterdata/services','/reports/top-metrics','/knowledge-base/checklists','/webshop/admin','/admin/access-control','/signage','/signage/appearance','/kiosk','/hr/positions'];
const scopedMenuRoutes=['/appointments/calendar','/workorders','/modules/team/timetable','/knowledge-base/checklists','/finance','/warehouse','/warehouse/products','/warehouse/operations','/warehouse/lots'];
const failures=[];
...
for(const route of critical){const exact=hasRoute(route);const customerDynamic=route.startsWith('/modules/customers/')&&hasDynamicCustomerRoute();if(!exact&&!customerDynamic)failures.push(`A routerben hiányzik: ${route}`)}
...
if(routeMatches.length!==184)failures.push(`A moduláris router route-száma megváltozott: várt 184, aktuális ${routeMatches.length}. Ellenőrizd, hogy route nem veszett-e el.`);
...
for(const moduleName of ['publicRoutes','bookingRoutes','hrRoutes','financeRoutes','inventoryRoutes','adminRoutes'])if(!routeAggregator.includes(moduleName))failures.push(`A route aggregátorból hiányzik: ${moduleName}`);
...
const workflowDir='.github/workflows';if(fs.existsSync(workflowDir)){for(const name of fs.readdirSync(workflowDir).filter(file=>/\.ya?ml$/i.test(file))){const workflow=fs.readFileSync(path.join(workflowDir,name),'utf8');const writesContents=/contents\s*:\s*write/i.test(workflow);const pushesMain=/git\s+push[^\n]*(?:HEAD:main|origin\s+main|refs\/heads\/main)/i.test(workflow);if(writesContents&&pushesMain)failures.push(`Közvetlen main-forrásmódosító workflow tiltott: ${name}`)}}
const forbiddenDashboardFallback=/function\s+FallbackRedirect\s*\(\s*\)\s*\{[\s\S]{0,320}<Navigate\s+to=\{getToken\(\)\?HOME_PATH/.test(routeAccess);if(forbiddenDashboardFallback)failures.push('Az ismeretlen route még mindig csendben az irányítópultra irányít.');
if(!/MENU_CACHE_KEY='kleo\.menu\.cache\.v\d+'/.test(sidebar))failures.push('A menü-cache kulcs nincs verziózva.');
if(failures.length){console.error('MENÜ / ROUTING ARCHITEKTÚRA AUDIT HIBA');failures.forEach(x=>console.error(' - '+x));process.exit(1)}
